...
const PayrollForm: React.FC<PayrollFormProps> = ({ payrollId }) => {
  ...
  useEffect(() => {
    const fetchPayroll = async () => {
      try {
        const response = await axios.get<Payroll>(` https://causal-jawfish-immune.ngrok-free.app/v1/api/payrolls/${payrollId}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        await axios.put(` https://causal-jawfish-immune.ngrok-free.app/v1/api/payrolls/${payroll.id}`, payroll);

...
const OrderHistory = () => {
  ...
  useEffect(() => {
    if (customerId) {
      axios.get(` https://causal-jawfish-immune.ngrok-free.app/v1/api/invoices/customer/${customerId}`)
        .then(response => {
  ...
  const fetchOrderDetails = (invoiceId) => {
    ...
    axios.get(` https://causal-jawfish-immune.ngrok-free.app/v1/api/invoice-details/invoice/${invoiceId}`)
      .then(response => {

...
const TablePayroll = () => {
  ...
  const fetchPayrolls = useCallback(async () => {
    try {
      const response = await axiosInstance.get(
        " https://causal-jawfish-immune.ngrok-free.app/v1/api/payrolls/group-by-working-period-status"
      );
  ...
  const fetchDetailedPayrolls = async (workingPeriod: string) => {
    try {
      const response = await axiosInstance.get(
        ` https://causal-jawfish-immune.ngrok-free.app/v1/api/payrolls/working-period?workingPeriod=${encodeURIComponent(
          workingPeriod
        )}`
      );
